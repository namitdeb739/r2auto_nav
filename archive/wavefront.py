...
class OccupancyGrid2d():
    ...
    def mapToWorld(self, mx, my):
        wx = self.map.info.origin.position.x + \
            (mx + 0.5) * self.map.info.resolution
        wy = self.map.info.origin.position.y + \
            (my + 0.5) * self.map.info.resolution

        return (wx, wy)

    def worldToMap(self, wx, wy):
        if (wx < self.map.info.origin.position.x
                or wy < self.map.info.origin.position.y):
            raise Exception("World coordinates out of bounds")

        mx = int((wx - self.map.info.origin.position.x)
                 / self.map.info.resolution)
        my = int((wy - self.map.info.origin.position.y)
                 / self.map.info.resolution)
...
def findFree(mx, my, costmap):
    ...
    while len(bfs) > 0:
        ...
        if costmap.getCost(loc.mapX, loc.mapY) \
    ...
    while len(mapPointQueue) > 0:
        ...
        for v in getNeighbors(p, costmap, fCache):
            if v.classification \
                ...
                if any(costmap.getCost(x.mapX, x.mapY)
                        == OccupancyGrid2d.CostValues.FreeSpace.value
...
def getNeighbors(point, costmap, fCache):
    ...
    for x in range(point.mapX - 1, point.mapX + 2):
        for y in range(point.mapY - 1, point.mapY + 2):
            if (x > 0 and x < costmap.getSizeX() and y > 0
                    and y < costmap.getSizeY()):
...
def isFrontierPoint(point, costmap, fCache):
    if costmap.getCost(point.mapX, point.mapY) \
            != OccupancyGrid2d.CostValues.NoInformation.value:
        return False
...
class WaypointFollowerTest(Node):
    ...
    def __init__(self):
        ...
        self.action_client = \
            ActionClient(self, FollowWaypoints, 'FollowWaypoints')
    ...
    def moveToFrontiers(self):
        ...
        largestDist = 0
        for f in frontiers:
            dist = math.sqrt(((f[0] - self.currentPose.position.x) ** 2) +
                             ((f[1] - self.currentPose.position.y)**2))
            if dist > largestDist:
                largestDist = dist
                location = [f]

        # worldFrontiers = \
        #        [self.costmap.mapToWorld(f[0], f[1]) for f in frontiers]
    ...
    def costmapCallback(self, msg):
        ...
        self.get_logger().info(f'Got Costmap \
                {len(getFrontier(None, self.costmap, self.get_logger()))}')
    ...
    def dumpCostmap(self):
        ...
        self.get_logger().info(f'costmap resolution \
                {costmap.specs.resolution}')
    ...
    def run(self, block):
        ...
        while not self.action_client.wait_for_server(timeout_sec=1.0):
            self.info_msg("'FollowWaypoints' action server not available, \
                    waiting...")
        ...
        if len(result.missed_waypoints) > 0:
            self.info_msg('Goal failed to process all waypoints, missed {0} \
                    wps.'.format(len(result.missed_waypoints)))
            return False
